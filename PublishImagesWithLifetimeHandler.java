...
public class PublishImagesWithLifetimeHandler implements HttpHandler {

    private static final String FINISHED_TO_PUBLISH_IMAGE_WITH_LIFETIME_AND_RETURN_DEDUPLICATED_ID =
            "FINISHED to publishImagesWithLifetimeAndReturnDeduplicatedIds(%s, %s, %s) RETURNED[%s]";

    private final ImageManagerHanlderUtilities imageManagerHanlderUtilities;

    public PublishImagesWithLifetimeHandler(ImageManagerHanlderUtilities imageManagerHanlderUtilities) {
    ...
    public void handleRequest(HttpServerExchange exchange) throws Exception {
        ...
        for (String imageId : imageIds.split("[,;]")) {
            ...
            b.put(imgId, imageManagerHanlderUtilities.publishImageWithLifetimeAndReturnDeduplicatedId(imgId, validFrom, validTill));
        }
        ...
        imageManagerHanlderUtilities.sendObjectAsJsonResponse(
                ...
                String.format(FINISHED_TO_PUBLISH_IMAGE_WITH_LIFETIME_AND_RETURN_DEDUPLICATED_ID,
                        imageIds, validFrom, validTill, b.build())
        );
    }
}
